...
if (isset($argv[1]) && file_exists($argv[1]) && ($buffer = file_get_contents($argv[1])) && 
    preg_match('/Copy:\s+([0-9\.]+)\s/msU', $buffer, $copy) && 
    preg_match('/Scale:\s+([0-9\.]+)\s/msU', $buffer, $scale) && 
    preg_match('/Add:\s+([0-9\.]+)\s/msU', $buffer, $add) && 
    preg_match('/Triad:\s+([0-9\.]+)\s/msU', $buffer, $triad)) {
  ...
  if (preg_match_all('/Threads requested = ([0-9]+)\s/msU', $buffer, $threads) && 
    ...
    foreach($threads[1] as $i => $t) {
      if (isset($triad[1][$i+1]) && is_numeric($tp = $triad[1][$i+1])) {
    ...
    foreach($triad_threaded as $threads => $triad) printf("stream_triad%d=%s\n", $threads, $triad);
